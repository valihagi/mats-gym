...
from scenic.simulators.carla.utils.utils import (
    scenicToCarlaLocation,
    carlaToScenicPosition,
    scenicToCarlaRotation,
)
...
class ScenicSimulation(AtomicBehavior):
    ...
    def update(self):
        ...
        if termination_reason is not None:
            self.logger.debug(
                f"{self.__class__.__name__}: Terminating due to {termination_type}: {termination_reason}"
            )
        ...
        termination_reason = (
            self.dynamic_scenario._checkSimulationTerminationConditions()
        )
        if termination_reason is not None:
            self.logger.debug(
                f"{self.__class__.__name__}: Terminating due to simulation termination condition: {termination_reason}"
            )
            return py_trees.common.Status.SUCCESS
        if self.max_time_steps and self._num_steps >= self.max_time_steps:
            self.logger.debug(
                f"{self.__class__.__name__}: Reached maximum number of steps: {self.max_time_steps}"
            )
            return py_trees.common.Status.SUCCESS
        ...
        for agent in self.simulation.scheduleForAgents():
            ...
            if isinstance(actions, EndSimulationAction):
                self.logger.debug(
                    f"{self.__class__.__name__}: Terminating due to agent {agent} action: {actions}"
                )
                return py_trees.common.Status.SUCCESS
...
class ScenicScenario(BasicScenario):
    def __init__(
        ...
        self._ego_vehicle_names = [
            ego_vehicle.rolename for ego_vehicle in config.ego_vehicles
        ] or ["hero"]
    ...
    def _initialize_actors(self, config: ScenarioConfiguration):
        ...
        controllable_objects = [
            o
            for o in self._simulation.objects
            if isinstance(o.carlaActor, carla.Vehicle)
            or isinstance(o.carlaActor, carla.Walker)
        ]
            ...
            if actor.attributes["role_name"] in self._ego_vehicle_names:
                actor_config = next(
                    (
                        ...
                        if ego_vehicle.rolename == actor.attributes["role_name"]
                    ),
                    None,
                )
                if hasattr(obj, "route"):
                    actor_config.route = self._generate_route(
                        actor=actor, route=obj.route
                    )
                ...
                logging.debug(
                    f"Created non-ego actor '{actor.attributes['role_name']}'."
                )

    def _generate_route(
        self, actor: carla.Vehicle, route: list[Lane], resolution=1.0
        ...
        waypoints = [(actor.get_transform(), RoadOption.LANEFOLLOW)]
        ...
        for point in points:
            ...
            if self._debug_mode:
                ...
                world.debug.draw_point(
                    loc, size=0.1, color=carla.Color(0, 0, 255), life_time=1000
                )

            # If the waypoint is ahead of the vehicle, add it to the route
            is_ahead = is_within_distance_ahead(
                wp.transform, actor.get_transform(), max_distance=np.inf
            )
    ...
    def _create_behavior(self):
        scenic_behavior = ScenicSimulation(
            simulation=self._simulation, max_time_steps=self._max_time_steps
        )
    ...
    def _create_test_criteria(self):
        ...
        criteria = py_trees.composites.Parallel(
            name="ScenicCriteria", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL
        )
        for actor in self.ego_vehicles:
            actor_config = next(
                filter(
                    lambda x: x.rolename == actor.attributes["role_name"],
                    self.config.ego_vehicles,
                )
            )
    ...
    def _create_tests_for_actor(
        self, actor: carla.Vehicle, route=None
    ) -> typing.List[atomic_criteria.Criterion]:
